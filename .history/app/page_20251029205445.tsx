...
// A helper component for icons (since Material Symbols aren't standard in React)
const Icon = ({ name, className = "w-5 h-5" }) => {
  const icons = {
    ...
    mosque: <Mosque className={className} />,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      // Handle timer completion (e.g., switch mode, play sound)
      setIsTimerRunning(false);
      // This is where you'd increment completedPomodoros for the active task
      return;
    }
  ...
  useEffect(() => {
    // This is where you'd get geolocation...
    // navigator.geolocation.getCurrentPosition(pos => { ... });
    // ...then fetch prayer times from an API
    // fetch(`https://api.aladhan.com/v1/timingsByCity?city=...&country=...`)
    //   .then(res => res.json())
    //   .then(data => {
    //     setPrayerTimes(data.data.timings);
    //     // Add logic to calculate next prayer
    //   });
  ...
  const handlePomodoroControl = (action) => {
    switch (action) {
      ...
      case 'reset':
        ...
        break;
      case 'skip':
        // Logic to skip to break or next focus
        if (timerMode === 'focus') {
  ...
  return (
    <div className="flex h-screen w-full bg-background-light dark:bg-background-dark font-display text-text-primary dark:text-slate-200">
      ...
      {/* --- Sidebar (from code (1).html & code (4).html) --- */}
      <nav className="flex h-screen min-h-[700px] w-64 flex-col justify-between bg-white p-4 dark:bg-[#192730] border-r border-slate-200 dark:border-slate-700">
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-4 border-t border-slate-200 dark:border-slate-700 pt-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3 rounded-lg px-3 py-2 text-gray-700 dark:text-slate-300">
                <Icon name="sunrise" className="text-xl" />
                <p className="flex-1 text-sm font-medium">Fajr</p>
                <span className="text-xs text-gray-500 dark:text-slate-400">{prayerTimes.Fajr}</span>
              </div>
              <div className={`flex items-center gap-3 rounded-lg px-3 py-2 ${nextPrayer.name === 'Dhuhr' ? 'bg-primary/10 text-primary' : 'text-gray-700 dark:text-slate-300'}`}>
                <Icon name="sun" className="text-xl" />
                <p className="flex-1 text-sm font-bold">Dhuhr</p>
                <span className="text-xs font-semibold">{prayerTimes.Dhuhr}</span>
              </div>
              <div className="flex items-center gap-3 rounded-lg px-3 py-2 text-gray-700 dark:text-slate-300">
                <Icon name="sun" className="text-xl opacity-70" />
                <p className="flex-1 text-sm font-medium">Asr</p>
                <span className="text-xs text-gray-500 dark:text-slate-400">{prayerTimes.Asr}</span>
              </div>
              <div className="flex items-center gap-3 rounded-lg px-3 py-2 text-gray-700 dark:text-slate-300">
                <Icon name="sunset" className="text-xl" />
                <p className="flex-1 text-sm font-medium">Maghrib</p>
                <span className="text-xs text-gray-500 dark:text-slate-400">{prayerTimes.Maghrib}</span>
              </div>
              <div className="flex items-center gap-3 rounded-lg px-3 py-2 text-gray-700 dark:text-slate-300">
                <Icon name="moon" className="text-xl" />
                <p className="flex-1 text-sm font-medium">Isha</p>
                <span className="text-xs text-gray-500 dark:text-slate-400">{prayerTimes.Isha}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="grid flex-1 grid-cols-12 gap-6 p-6 overflow-y-auto">
        ...
        <section className="col-span-12 flex flex-col gap-4 md:col-span-12 lg:col-span-7">
          ...
          {/* Add Task Input (from code (1).html) */}
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {tasks.map(task => (
                  <tr 
                    key={task.id} 
                    className={`border-t border-slate-200 dark:border-slate-700 hover:bg-primary/10 dark:hover:bg-primary/20 cursor-pointer ${selectedTask?.id === task.id ? 'bg-primary-light dark:bg-primary/30' : ''}`}
                  >
        ...
        {/* Edit Task Panel (from code.html) */}
      ...
      {/* Pomodoro Timer Modal (from code (2).html) */}
      {isTimerModalOpen && (
      ...
      {/* Prayer Alert Modal (from code (3).html) */}
      {isPrayerAlertOpen && (
            ...
            <div className="flex flex-col gap-3">
              <button 
                ...
                onClick={() => {
                  ...
                  handleStartPomodoro(tasks.find(t => !t.isComplete) || tasks[0]); // Start pomodoro for next available task
                }}
              >
      ...
      {/* Settings Modal (from code (4).html) */}
      {isSettingsModalOpen && (
            ...
            <div className="flex flex-col gap-10">
              ...
              <section>
                ...
                <div className="mt-6 flex flex-col gap-6">
                  {/* ... (Auto-detect toggle and inputs from code (4).html) ... */}
