...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    const db = getDb();
    const task = db.prepare('SELECT * FROM tasks WHERE id = ?').get(parseInt(params.id)) as Task;
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const existingTask = db.prepare('SELECT * FROM tasks WHERE id = ?').get(parseInt(params.id)) as Task;
    if (!existingTask) {
      return NextResponse.json({ error: 'Task not found' }, { status: 404 });
    }
    ...
    const updatedTask = db.prepare('SELECT * FROM tasks WHERE id = ?').get(parseInt(params.id)) as Task;
